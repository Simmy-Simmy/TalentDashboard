...
# Load the merged candidates JSON
with open('/sessions/gifted-upbeat-planck/mnt/talent-dashboard/candidates_merged.json', 'r') as f:
    data = json.load(f)

candidates = data['candidates']

# Generate CSV export
csv_output = '/sessions/gifted-upbeat-planck/mnt/talent-dashboard/candidates_merged.csv'
with open(csv_output, 'w', newline='', encoding='utf-8') as f:
    ...
    for cand in candidates:
        writer.writerow({
            ...
            'role': cand['role'],
            'office_location': cand['office_location'],
            'stage': cand['stage'],
            'status': cand['status'],
            'interview_score': cand['interview_score'],
            'risk_level': cand['risk_level'],
            'next_step': cand['next_step'],
            'last_updated': cand['last_updated'],
            'source': cand['source'],
            'linkedin_url': cand['linkedin_url']
        })

print(f"CSV export created: {csv_output}")

# Generate detailed mapping and field source document
mapping_doc = '''
...
with open('/sessions/gifted-upbeat-planck/mnt/talent-dashboard/DATA_EXTRACTION_REPORT.md', 'w') as f:
    f.write(mapping_doc)

print("Mapping and quality report created: DATA_EXTRACTION_REPORT.md")

# Create a detailed sample table
print("\n" + "="*150)
print("SAMPLE CANDIDATE DETAILS (First 5)")
print("="*150)

for i, cand in enumerate(candidates[:5], 1):
    print(f"\n{i}. {cand['candidate_name']}")
    print(f"   Role: {cand['role']}")
    print(f"   Office: {cand['office_location']}")
    print(f"   Stage: {cand['stage']}")
    print(f"   Status: {cand['status']}")
    print(f"   Score: {cand['interview_score']}")
    print(f"   Risk Level: {cand['risk_level']}")
    print(f"   Last Updated: {cand['last_updated']}")
    print(f"   Multiple Interviews: {cand['num_interviews']}")
